...
「関連Issue・Linear・研究文書」には、必要に応じてLinear、GitHub Issue、Decision、OQ、要件ID、その他の関連文書を記録する。

研究仕様への影響がない場合は「なし」と明記する。未検証事項がない場合も「なし」と明記する。検証に失敗した場合や未実行の検証がある場合は、失敗内容または未実行理由を記録する。
