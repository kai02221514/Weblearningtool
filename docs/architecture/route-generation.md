...
> 旧`docs/route-generation-spec.md`はOQ-005検討用たたき台（D-012）であり、本文書は同たたき台の項目別採否判断を経て確定した後継仕様である。
...
- 修正採用したルート生成規則: RT-02（`dataVersion`と`generatedAt`を必須化し、`generatedAt`のみ決定性比較対象外）、RT-07（復習解除を原因単位で管理）。
...
| RT-02 | 出力契約の必須項目 | 修正採用。版情報を必須化 | D-012、旧spec | A: 旧spec型を全採用 / B: §6の必須項目で再定義 | B | `status`・順序・理由は採用、`routeId`はOQ-009確定まで導入せず、`generatedAt`は必須メタ情報かつ決定性対象外と明示 | 出力＝説明可能性・再現性の契約 | 純粋関数の戻り値 | ログ化すれば根拠を追跡可能 | 研究者本人 |
...
interface RouteRecommendation {
  ...
  warnings: string[]
  generatedAt: string        // 必須メタ情報。決定性の比較対象外
}
```

- 必須項目: `specVersion`、`catalogVersion`、`dataVersion`、`status`、`nextNodeId`、`route`（各要素の`nodeId`・`order`・`reasons`）、`presentedCount`、`warnings`、`generatedAt`。
...
- `generatedAt`は必須だが、決定性比較の対象外とする。純粋なルート判断では現在時刻を読まず、生成時刻メタ情報として付与する。
...
2. OQ-009依存事項: S群・A群の収集要否、年齢・職業の追加取得、入力スナップショットと出力の永続保存、保持期間、削除手順、同意UI、評価ログ保存、`routeId`導入要否。
