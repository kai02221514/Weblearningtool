...
> DG-08（診断必須化・誘導タイミング・完了状態保存）はD-022で確定した。詳細な保存契約は`docs/operations/research-data-management.md`に従う。
...
- 後続確定した診断規則: DG-08。D-022により初回診断を必須化し、保存成功をDashboard遷移の条件とする。
...
| DG-07 | 欠損・未回答時の開始ノード | 採用。欠損時は`html-000` | 06-implementation-status | A: 初回診断完了まで学習不可 / B: html-000から開始し警告を記録 | B（純粋関数の防御的フォールバック） | 2026-07-03時点は最保守側の開始を採用した。D-022後は通常導線をDG-08で遮断し、本規則は異常入力への決定的な防御として維持する | フォールバック1分岐。通常UIはDG-08を優先 | 欠損有無を警告で識別可能 | 研究者本人 |
| DG-08 | 診断必須化・誘導タイミング | D-022で採用。初回診断と保存成功を必須化 | D-022、research-data-management | A: 初回サインイン後に必須 / B: 任意（未回答はDG-07適用） | A | 診断を経ない固定導線と再ログイン後の状態不整合を防ぎ、開始判定とルート生成へ同じK群回答を渡す | 保存成功前はDashboard・学習画面への遷移を拒否し、失敗時は回答を保持して再試行 | K群3項目の最新状態、診断版、完了・更新日時を本人単位で保存・復元 | 評価手順のうち診断→提示の順序を固定。参加者同意は別ゲート | 研究者本人 |
...
| RT-05 | 競合時の優先順位 | 採用。6段階優先順位を使用 | OQ-005 | A: 旧spec順（進行中>前提>エラー>テスト>振り返り>診断） / B: §7の6段階（進行中>エラー主推薦>テスト不合格>エラー補助推薦>振り返り>新規学習） | B | 客観的・直近の証拠を自己申告より優先。学習開始後は実測（エラー・テスト）が静的診断より新しい状態情報を持つ（知識追跡の逐次更新の考え方: Corbett & Anderson 1994）。前提補完は優先度でなく順序変換として扱う | 個別化の判断規則そのもの | 固定優先度の比較関数 | 優先順位自体が評価・考察対象 | 研究者本人（将来の利用評価、またはD-021の中間発表時点の主張範囲を超える学習効果・妥当性・優位性の主張へ用いる場合は指導教員確認推奨） |
...
- [確定事項] D-022により、初回診断保存ではS群・A群を保存しない。将来の研究データとしての収集要否と年齢・職業の追加取得はOQ-009および評価計画の未確定事項として残す。
...
- [確定事項] D-022により初回診断を必須とし、K群3項目の保存成功後だけDashboardへ遷移する。診断完了前はDashboardおよび学習画面へ進ませない。
- [確定事項] 保存失敗時は入力済みのK群回答を画面内に保持し、Dashboardへ遷移せず再試行可能とする。
- [確定事項] 明示的な再ログイン後は互換な保存済みK群回答を復元し、開始ノード判定とルート生成へ反映する。欠損、未知値、非互換版は未完了扱いとして再診断させる。
- 保存項目、版互換性、本人アクセス、非本番環境の詳細契約は`docs/operations/research-data-management.md` §1.4に従う。参加者同意、保持・撤回・削除、研究者用アクセス等はD-022の対象外であり、別ゲートとして未確定のまま維持する。
...
以下のうちDG-08はD-022で確定した。その他はOQ-009または後続確認へ残す。

1. DG-08: 初回診断を必須とし、K群3項目の保存成功後だけDashboardへ遷移する。保存失敗時は回答を保持して再試行させ、明示的な再ログイン後は互換な保存済み回答を復元する。欠損・未知値・非互換版は未完了として再診断させる。
2. OQ-009依存事項: S群・A群の将来の収集要否、年齢・職業の追加取得、入力スナップショットと`RouteGenerationResult`の永続保存、`generatedAt`の付与・保存形式、保持期間、削除手順、同意UI、評価ログ保存、`routeId`導入要否。
3. 指導教員確認推奨事項: 診断項目が対象者条件や将来の利用評価へ与える影響、および開始2候補・上位3件提示・6段階優先順位を、D-021の中間発表時点の主張範囲を超えて、学習効果・妥当性・優位性の主張へ用いる場合の範囲。
4. OQ-001はD-021で解消され、本仕様は正式な主RQのシステムレベル検証対象となった。利用者に対する明確さ・納得感は将来評価の副次RQであり、本仕様だけで肯定的な結論を出さない。
...
| 1.0補足 | 2026-09-05 | D-022に基づきDG-08を確定。初回診断必須化、保存成功ゲート、再ログイン後復元、K群3項目の最小保存境界を同期 | 承認済み仕様の補足 |
