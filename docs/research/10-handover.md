...
- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断はPR #1で`main`へ反映済み、Linear上もDone。KAI-11/OQ-006は研究者レビュー済み仕様案まで進んだが、指導教員確認と予備試行の前であり、解消済みではない。
...
- Phase 3の優先順位:
  1. `KAI-11` OQ-006 確認テスト規則
  2. `KAI-12` OQ-009 研究データ管理仕様
  3. `KAI-13` テスト・型検査基盤
...
- Phase 3進行状況（2026-07-04）: KAI-9/KAI-10の研究者判断を`docs/architecture/route-generation.md`（`route-spec/1.0`、状態: 承認済み仕様）、`01-confirmed-decisions.md`、`02-open-questions.md`、`09-decision-log.md`へPR #1で反映済み。D-016でOQ-004は条件付き解消、D-017でOQ-005は解消した。KAI-11/OQ-006は`docs/architecture/quiz-assessment.md`に研究者レビュー済み仕様案（`quiz-assessment-proposal/0.2`）を作成済みであり、QA-01〜QA-18の採否を反映済みである。ただし、指導教員確認、予備試行、D-018正式追加、KAI-11完了判断の前であり、OQ-006は未解消である。
...
- GitHub/Linear同期状況（2026-07-03確認）: PR `#1`（`docs/phase3-route-spec-proposal` -> `main`）はmerged。merge commitは`1a6216b01e9d49315f9bee84e67c93b449b44432`。Linear `KAI-9`/`KAI-10`は`Done`、`KAI-11`は`In Progress`。
- 未完了事項: OQ-006の正式解消、指導教員確認、予備試行、D-018正式追加、OQ-009、`routeGenerator`、ノード別確認テスト・教材、同意・永続化・評価ログ、テスト・型検査・CI基盤、P-06/P-07/P-09の保留判断。
- 次の最小作業: KAI-11 / OQ-006の指導教員確認用1ページ資料を作成する。その後、KAI-15で`html-010`、`html-021`、`css-011`の3ノード分の問題試作を行い、予備試行後にD-018、OQ-006、KAI-11完了判断を行う。
- 指導教員確認が必要なIssue: `KAI-12`、`KAI-16`は必須。`KAI-11`、`KAI-15`は評価設計への影響により推奨。KAI-9/KAI-10で採用した開始2候補、上位3件提示、6段階優先順位を研究主張へ使う場合も確認推奨。

[禁止] OQ-006、OQ-009の未確定事項を、Codexまたは他AIが実装上の既定値で補完してはならない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験・2形式限定・合格後再受験なしは研究者レビュー済み仕様案であり、指導教員確認、予備試行、D-018正式追加の前に正式確定事項として扱わない。Quiz実装とrouteGenerator実装はまだ開始しない。
...
[確定事項] Codexは確定済み仕様に基づく実装を担当する。ただし、routeGenerator実装はKAI-11とKAI-12完了後、またはロードマップ上のゲート確認後に着手する。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`と`KAI-10`は研究者判断を`main`へ反映しLinear上も完了済みである。`KAI-11`は研究者レビュー済み仕様案まで進んだため、次の最小作業は指導教員確認用1ページ資料の作成である。

- `KAI-9`: OQ-004 診断規則と開始ノード規則を確定する。状態: 完了。DG-08のみOQ-009へ移管。
- `KAI-10`: OQ-005 ルート生成規則と出力契約を確定する。状態: 完了。
- `KAI-11`: OQ-006 確認テスト規則を確定する。状態: In Progress。研究者レビュー済み仕様案は`quiz-assessment-proposal/0.2`。
...
- 前提: `KAI-9`と`KAI-10`の仕様判断が`main`へ反映され、`KAI-11`と`KAI-12`完了後、またはロードマップ上のゲート確認後に着手する。
...
- 現在の仕様案: `docs/architecture/quiz-assessment.md`（`quiz-assessment-proposal/0.2`、研究者レビュー済み仕様案）
...
- KAI-15の範囲: 実際の問題文、選択肢、正答、許容解、解説、問題ごとの関連前提ノード、教材本文との整合、短いコード補完の正規化ルール、初期`questionSetVersion`を整備する。
- 前提: 指導教員確認、予備試行、D-018正式追加、OQ-006正式解消、KAI-11完了判断が終わるまで、Quiz実装やrouteGenerator実装へ進めない。
- 次作業: 指導教員確認用1ページ資料を作成する。その後、KAI-15で`html-010`、`html-021`、`css-011`の3ノード分の問題試作を行う。
- 受入条件: 得点、合否、誤答、対象ノード、問題集合版、受験回数を取得できる。
...
- KAI-11の研究者レビュー済み仕様案を、指導教員確認、予備試行、D-018正式追加の前に確定事項として扱う。

## 引継ぎ資料だけで開始できる作業範囲

- [確認済み事実] 新規担当者は、本パッケージだけで研究目的、MVP境界、確定事項、未確定事項、次に仕様化すべき論点を把握し、KAI-11/OQ-006の仕様検討を開始できる。コード変更を伴う作業開始には、最新リポジトリの取得と実装状態の再確認が必要である。
