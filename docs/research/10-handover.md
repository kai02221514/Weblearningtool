...
- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断反映済み
...
- Phase 3進行状況（2026-07-03）: KAI-9/KAI-10の研究者判断を`docs/architecture/route-generation.md`（`route-spec/1.0`、状態: 承認済み仕様）、`01-confirmed-decisions.md`、`02-open-questions.md`、`09-decision-log.md`へ反映済み。D-016でOQ-004は条件付き解消、D-017でOQ-005は解消した。
- KAI-9の診断規則はDG-08を除き確定。DG-08は診断必須化、誘導タイミング、診断完了状態保存に関わるためOQ-009/KAI-12へ移管した。
- KAI-10は確定。RT-02は版情報必須化、RT-07は原因単位の復習解除へ修正済み。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`と`KAI-10`は研究者判断反映済みであり、次の最小作業は`KAI-11`である。
...
- 前提: `KAI-9`と`KAI-10`完了に加え、`KAI-11`と`KAI-12`完了後、またはロードマップ上のゲート確認後に着手する。
