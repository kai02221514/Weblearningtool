...
- 現在の優先順位:
  1. 現在作業: `KAI-24` Draft PR #17のレビューと監査指摘解消。mainマージ後に完了判定する。
  2. 研究判断ゲート: `KAI-12` / OQ-009。研究データ管理、同意、保存、削除、アクセス権限、評価ログを確定する。
  3. 独立実装候補: `KAI-13`のテスト・型検査基盤の継続整備。ただしLinear本文に残るCI必須化タイミングは着手時に確認する。
  4. 条件付き候補: `KAI-14`はMVP外6エラーの扱いに軽い判断が必要、`KAI-25`は課題内容・完了条件等の根拠確認が必要なため、未確定判断を解消してから着手可否を決める。
...
- 次の最小作業:
  1. KAI-24: Draft PR #17のレビュー、必要修正、mainマージ後の最終確認。
  2. 研究判断ゲートとしてKAI-12 / OQ-009を確定する。保存・同意・評価ログ実装は確定前に開始しない。
  3. 並行可能な独立候補はKAI-13とし、CI必須化タイミングを確認した範囲だけ進める。
  4. KAI-14、KAI-25、予備試行は、それぞれの未確定判断・依存・レビュー条件を確認してから次作業として選定する。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`、`KAI-10`、`KAI-11`は研究者判断を正本文書へ反映しLinear上も完了済みである。次の研究判断ゲートは`KAI-12`である。KAI-24はDraft PR #17で統合検証をレビュー中であり、mainマージ前のため完了扱いしない。KAI-13は未確定研究判断を伴わない範囲の独立実装候補、KAI-14とKAI-25はLinear本文に明記された判断・依存条件を確認後の条件付き候補とする。
...
- KAI-15の残作業: Draft PR #17のレビュー・mainマージ後のKAI-24完了判定、KAI-12確定後の保存実装、条件確認後の実践課題整備、3〜5名程度の予備試行。
