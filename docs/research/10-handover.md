...
- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断はPR #1で`main`へ反映済み、Linear上もDone。KAI-11/OQ-006は研究者採否と指導教員確認を経て初期仕様確定として解消済みであり、D-018を正式追加済み。KAI-15はIn Progressで、予備試行用3ノード9問と教材案は研究者レビュー済み・予備試行前である。3ノード9問の型付きデータ化と構造検証テストはKAI-20として追加済みであり、D-020で短いコード補完3問の許容解・正規化規則も研究者判断として確定済みである。KAI-22でUI非依存の採点・正規化純粋関数を追加し、KAI-21でQuiz UIを対象3ノードの型付きデータとKAI-22採点関数へ接続した。KAI-23でメモリ内の再受験制御と試行結果モデルを追加した。KAI-24では予備試行前統合検証のための横断テストと検証記録を追加した。ただし、KAI-15全体は完了ではない。
...
- 現在の優先順位:
  ...
  2. `KAI-24` 予備試行前のクイズ統合検証
  3. `KAI-12` OQ-009 研究データ管理仕様
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`、`KAI-10`、`KAI-11`は研究者判断を正本文書へ反映しLinear上も完了済みである。次の仕様確定優先Issueは`KAI-12`である。KAI-15ではD-020により許容解・正規化の研究者判断が完了し、KAI-22で採点処理を追加し、KAI-21で対象3ノードのQuiz UI接続を追加し、KAI-23でメモリ内の再受験制御を追加したため、次はKAI-24の統合検証へ進む。
...
- KAI-15の残作業: KAI-12確定後の保存実装、KAI-24での予備試行前統合検証、3〜5名程度の予備試行。
