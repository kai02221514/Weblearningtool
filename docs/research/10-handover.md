...
- 最新確認時点（2026-07-03）の`main`: `e3c8378a9889d854b966960eea97ad8d237a9be3`
- 実装状態の基準コミット: `42c298ca2f1ed084121de0decb3df18be590a9eb`
- 参照日: 2026-07-03

[注意] `1a8efb5...`と`0f09e5...`は2026-07-02監査時点の旧スナップショットである。`42c298c...`はコード実装状態を再確認した基準コミットであり、`e3c8378...`はPhase 2/2.5文書クローズ後の最新確認`main`である。新規担当者は作業開始時にさらに最新`main`との差分を確認する。

## 現在地

- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断はPRブランチへ反映済みで、`main`反映とLinear完了化はPRマージ後に行う
...
- Phase 3進行状況（2026-07-03）: KAI-9/KAI-10の研究者判断を`docs/architecture/route-generation.md`（`route-spec/1.0`、状態: 承認済み仕様）、`01-confirmed-decisions.md`、`02-open-questions.md`、`09-decision-log.md`へPRブランチ上で反映済み。D-016でOQ-004は条件付き解消、D-017でOQ-005は解消した。
...
- GitHub/Linear同期状況（2026-07-03確認）: PR `#1`（`docs/phase3-route-spec-proposal` -> `main`）はopen・mergeable・status checksなし。Linear `KAI-9`/`KAI-10`は`Backlog`のため、PRマージと`main`反映を確認するまで運用上の完了扱いにしない。
- 未完了事項: OQ-006、OQ-009、`routeGenerator`、ノード別確認テスト・教材、同意・永続化・評価ログ、テスト・型検査・CI基盤、P-06/P-07/P-09の保留判断。
- 指導教員確認が必要なIssue: `KAI-12`、`KAI-16`は必須。`KAI-11`、`KAI-15`は評価設計への影響により推奨。KAI-9/KAI-10で採用した開始2候補、上位3件提示、6段階優先順位を研究主張へ使う場合も確認推奨。

[禁止] OQ-006、OQ-009の未確定事項を、Codexまたは他AIが実装上の既定値で補完してはならない。routeGenerator実装はまだ開始しない。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`と`KAI-10`は研究者判断をPRブランチへ反映済みであり、PRマージとLinear完了化後の次の最小作業は`KAI-11`である。

- `KAI-9`: OQ-004 診断規則と開始ノード規則を確定する。状態: 内容確定・PRマージ/Linear完了化待ち。DG-08のみOQ-009へ移管。
- `KAI-10`: OQ-005 ルート生成規則と出力契約を確定する。状態: 内容確定・PRマージ/Linear完了化待ち。
- `KAI-11`: OQ-006 確認テスト規則を確定する。
- `KAI-12`: OQ-009 研究データ管理仕様を確定する。DG-08、同意、保存、評価ログに関係する。

### `KAI-9` / `KAI-10`: ルート生成判断規則の確定

- 状態: [内容確定・PRマージ/Linear完了化待ち]
- 目的:
