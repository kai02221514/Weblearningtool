...
- KAI-31判断状態: 2026-09-06の研究者本人の明示判断をD-023として反映した。表示名は研究分析に使わない通常運用データ、唯一の正本は型付き`public.profiles.display_name`、emailの正本はSupabase Authとし、型・更新・削除・signup完了境界、KV廃止方針を確定した。project ref `znfwkrhquegvlcmugkoe`は合成データ専用の非本番remote検証環境とし、Git管理、local/CI、Draft PR監査、明示許可を満たす変更だけmigration・Function deployを許可した。研究者本人はD-023とその確定契約に基づくKAI-32〜KAI-35の着手判断を指導教員から委任されており、この範囲で追加の指導教員承認を開始条件としない。これは指導教員によるD-023の技術内容の個別確認・承認やremote変更の包括許可を意味せず、変更ごとの研究者本人の明示許可は必須である。実在参加者データ、研究データ収集、予備試行、同意、保持、撤回・削除、本人対応表、研究者access/export、評価ログ、学内手続、KAI-12/OQ-009の残余、KAI-16へ一般化しない。後続KAI-32〜KAI-35を作成し、文書Draft PR [#40](https://github.com/kai02221514/Weblearningtool/pull/40)を作成した。KAI-31はPR監査中のIn Reviewとし、マージ・完了証跡登録前にDoneへ戻さない。コード・DB・remote Supabaseは未変更である
...
- 関連Issueの作業状態:
  ...
  12. `KAI-31`: In Review。D-023をDraft PR #40へ反映し、監査・マージ・完了証跡待ちである。監査完了前にDoneへ戻さない。
