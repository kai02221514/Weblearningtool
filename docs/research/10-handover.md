...
- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断はPR #1で`main`へ反映済み、Linear上もDone。KAI-11/OQ-006は研究者採否と指導教員確認を経て初期仕様確定として解消済みであり、D-018を正式追加済み。KAI-15はIn Progressで、予備試行用3ノード9問と教材案は研究者レビュー済み・予備試行前である。3ノード9問の型付きデータ化と構造検証テストはKAI-20として追加済みであり、D-020で短いコード補完3問の許容解・正規化規則も研究者判断として確定済みである。KAI-22でUI非依存の採点・正規化純粋関数を追加し、KAI-21でQuiz UIを対象3ノードの型付きデータとKAI-22採点関数へ接続した。ただし、KAI-15全体は完了ではない。
...
- 未完了事項: 予備試行、OQ-009、`routeGenerator`、ノード別確認テストの再受験制御、教材UI組み込み、同意・永続化・評価ログ、テスト基盤の継続整備、P-06/P-07/P-09の保留判断。
- 次の最小作業:
  1. KAI-23: 再受験制御
  2. KAI-12確定後の保存実装
  3. KAI-24: 予備試行前統合検証
...
[禁止] OQ-009の未確定事項、再受験制御、保存、評価ログを、Codexまたは他AIが実装上の既定値で補完してはならない。KAI-15の予備試行用3ノード9問に関する短いコード補完の許容解・正規化規則はD-020に従い、未記載の許容解を独自に追加しない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験・2形式限定・合格後再受験なしはD-018に基づく初期仕様だが、予備試行結果に基づいて見直し得る。Quiz UI接続はKAI-21で対象3ノードに限定して実装済みであり、次はKAI-23の再受験制御である。routeGenerator実装はKAI-12完了後、またはロードマップ上のゲート確認後に着手する。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`、`KAI-10`、`KAI-11`は研究者判断を正本文書へ反映しLinear上も完了済みである。次の仕様確定優先Issueは`KAI-12`である。KAI-15ではD-020により許容解・正規化の研究者判断が完了し、KAI-22で採点処理を追加し、KAI-21で対象3ノードのQuiz UI接続を追加したため、次はKAI-23の再受験制御、KAI-24の統合検証へ進む。
...
- KAI-15の残作業: KAI-23での再受験制御、KAI-12確定後の保存実装、KAI-24での予備試行前統合検証、3〜5名程度の予備試行。
