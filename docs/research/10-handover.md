...
- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断はPR #1で`main`へ反映済み、Linear上もDone。KAI-11/OQ-006は研究者採否と指導教員確認を経て初期仕様確定として解消済みであり、D-018を正式追加済み。KAI-15はIn Progressで、予備試行用3ノード9問と教材案は研究者レビュー済み・予備試行前である。3ノード9問の型付きデータ化と構造検証テストはKAI-20として追加済みであり、D-020で短いコード補完3問の許容解・正規化規則も研究者判断として確定済みである。ただし、KAI-15全体は完了ではない。
...
- 現在の優先順位:
  ...
  4. `KAI-23` 再受験制御
  5. `KAI-13` テスト・型検査基盤の継続整備
  6. `KAI-14` errorMappings参照切れ整理
...
- Phase 3進行状況（2026-07-06）: KAI-9/KAI-10の研究者判断を`docs/architecture/route-generation.md`（`route-spec/1.0`、状態: 承認済み仕様）、`01-confirmed-decisions.md`、`02-open-questions.md`、`09-decision-log.md`へPR #1で反映済み。D-016でOQ-004は条件付き解消、D-017でOQ-005は解消した。KAI-11/OQ-006は`docs/architecture/quiz-assessment.md`の確認テスト規則を指導教員承認済み初期仕様として反映済みであり、D-018を正式追加済みである。KAI-11はLinear上Done。KAI-15では予備試行用3ノード9問と教材案が研究者レビュー済み・予備試行前になり、3ノード9問の型付きデータ化と構造検証テストを追加した。D-020で短いコード補完3問の許容解・正規化規則を確定し、KAI-22の仕様上の着手条件は満たされた。UI接続・採点実装・再受験制御・保存・予備試行は未実施である。
...
- 未完了事項: 予備試行、OQ-009、`routeGenerator`、ノード別確認テストのUI接続・採点実装・再受験制御、教材UI組み込み、同意・永続化・評価ログ、テスト基盤の継続整備、P-06/P-07/P-09の保留判断。
- 次の最小作業:
  1. KAI-22: UIから独立した採点処理
  2. KAI-21: Quiz UIへのノード別クイズデータ接続
  3. KAI-23: 再受験制御
  4. KAI-12確定後の保存実装
  5. KAI-24: 予備試行前統合検証
  6. 3〜5名程度の予備試行
...
[禁止] OQ-009の未確定事項、採点実装、UI接続、再受験制御を、Codexまたは他AIが実装上の既定値で補完してはならない。KAI-15の予備試行用3ノード9問に関する短いコード補完の許容解・正規化規則はD-020に従い、未記載の許容解を独自に追加しない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験・2形式限定・合格後再受験なしはD-018に基づく初期仕様だが、予備試行結果に基づいて見直し得る。Quiz実装は、KAI-22の採点処理、KAI-21のUI接続、KAI-23の再受験制御の順で進める。routeGenerator実装はKAI-12完了後、またはロードマップ上のゲート確認後に着手する。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`、`KAI-10`、`KAI-11`は研究者判断を正本文書へ反映しLinear上も完了済みである。次の仕様確定優先Issueは`KAI-12`である。KAI-15ではD-020により許容解・正規化の研究者判断が完了したため、次はKAI-22の採点処理、KAI-21のUI接続、KAI-23の再受験制御、KAI-24の統合検証へ進む。
...
- KAI-15の残作業: KAI-22でのUIから独立した採点処理、KAI-21でのQuiz UIへのノード別接続、KAI-23での再受験制御、KAI-12確定後の保存実装、KAI-24での予備試行前統合検証、3〜5名程度の予備試行。
...
- [確認済み事実] 新規担当者は、本パッケージだけで研究目的、MVP境界、確定事項、未確定事項、次に仕様化すべき論点を把握し、D-020に基づくKAI-22の採点処理、KAI-21のUI接続、KAI-23の再受験制御、またはKAI-12/OQ-009の仕様検討を開始できる。コード変更を伴う作業開始には、最新リポジトリの取得と実装状態の再確認が必要である。
