...
- 現在の優先順位:
  ...
  2. `KAI-15` 許容解・正規化に関する研究者判断
  3. `KAI-22` UIから独立した採点処理
  4. `KAI-21` Quiz UIへのノード別クイズデータ接続
  5. `KAI-23` 再受験制御
  6. `KAI-13` テスト・型検査基盤の継続整備
  7. `KAI-14` errorMappings参照切れ整理
...
- 未完了事項: 予備試行、OQ-009、`routeGenerator`、ノード別確認テストのUI接続・採点実装・許容解処理・再受験制御、教材UI組み込み、同意・永続化・評価ログ、テスト基盤の継続整備、P-06/P-07/P-09の保留判断。
- 次の最小作業:
  1. 許容解・正規化に関する研究者判断
  2. KAI-22: UIから独立した採点処理
  3. KAI-21: Quiz UIへのノード別クイズデータ接続
  4. KAI-23: 再受験制御
  5. KAI-12確定後の保存実装
  6. KAI-24: 予備試行前統合検証
  7. 3〜5名程度の予備試行
...
[禁止] OQ-009の未確定事項、KAI-15の許容解正規化、採点実装、UI接続、再受験制御を、Codexまたは他AIが実装上の既定値で補完してはならない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験・2形式限定・合格後再受験なしはD-018に基づく初期仕様だが、予備試行結果に基づいて見直し得る。Quiz実装は、許容解・正規化の研究者判断後に、KAI-22の採点処理、KAI-21のUI接続、KAI-23の再受験制御の順で進める。routeGenerator実装はKAI-12完了後、またはロードマップ上のゲート確認後に着手する。
...
`docs/references/`および`docs/archive/`と`docs/research/`配下が矛盾する場合は、`docs/research/`配下と最新の有効なDecisionを優先する。コードは現在動いている事実を示すが、研究仕様の正解とはみなさない。
...
6. 作業内容に応じて `docs/architecture/` 配下の承認済み技術仕様
7. `docs/operations/ai-research-development-roadmap.md` と `docs/operations/codex-workflow.md`
8. 実装開始時はLinear Issueと対象仕様文書
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`、`KAI-10`、`KAI-11`は研究者判断を正本文書へ反映しLinear上も完了済みである。次の仕様確定優先Issueは`KAI-12`であり、KAI-15では許容解・正規化の研究者判断後に、KAI-22の採点処理、KAI-21のUI接続、KAI-23の再受験制御、KAI-24の統合検証へ進む。
...
- KAI-15の現在地: `html-010`、`html-021`、`css-011`の3ノード9問と教材案は研究者レビュー済み・予備試行前。3ノード9問の型付きデータ化と構造検証テストは追加済み。KAI-15はIn Progressで完了ではない。
- KAI-15の残作業: 許容解・正規化に関する研究者判断、KAI-22でのUIから独立した採点処理、KAI-21でのQuiz UIへのノード別接続、KAI-23での再受験制御、KAI-12確定後の保存実装、KAI-24での予備試行前統合検証、3〜5名程度の予備試行。
...
1. `09-decision-log.md` に理由・根拠・影響・再検討条件を追記する。
2. `01-confirmed-decisions.md` に現在有効な判断を反映する。
3. `02-open-questions.md` を必要に応じて更新する。
...
- [確認済み事実] 新規担当者は、本パッケージだけで研究目的、MVP境界、確定事項、未確定事項、次に仕様化すべき論点を把握し、KAI-15の許容解・正規化判断、KAI-22の採点処理、KAI-21のUI接続、KAI-23の再受験制御、またはKAI-12/OQ-009の仕様検討を開始できる。コード変更を伴う作業開始には、最新リポジトリの取得と実装状態の再確認が必要である。
