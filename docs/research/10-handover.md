...
- 現在の優先順位:
  ...
  2. `KAI-15` Quiz UI接続・採点仕様具体化・再受験制御・予備試行前統合検証
  3. `KAI-13` テスト・型検査基盤（lint/CIを含む継続整備）
...
- 未完了事項: 予備試行、OQ-009、`routeGenerator`、ノード別確認テストのUI接続・採点実装・許容解処理・再受験制御、教材UI組み込み、同意・永続化・評価ログ、lint/CIを含むテスト基盤全体、P-06/P-07/P-09の保留判断。
- 次の最小作業:
  1. Quiz UIへのノード別接続
  2. 採点・許容解正規化
  3. 再受験制御
...
[禁止] OQ-009の未確定事項、KAI-15の許容解正規化、採点実装、UI接続、再受験制御を、Codexまたは他AIが実装上の既定値で補完してはならない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験・2形式限定・合格後再受験なしはD-018に基づく初期仕様だが、予備試行結果に基づいて見直し得る。Quiz実装はKAI-15の次作業として、UI接続・採点・許容解処理・再受験制御を明示して進める。routeGenerator実装はKAI-12完了後、またはロードマップ上のゲート確認後に着手する。
...
[確認済み事実] `package.json`には `dev`、`typecheck`、`test`、`build`、`lint` スクリプトがある。KAI-20ではクイズデータ構造検証テストを追加した。lintとCI整備はKAI-13として継続する。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`、`KAI-10`、`KAI-11`は研究者判断を正本文書へ反映しLinear上も完了済みである。次の仕様確定優先Issueは`KAI-12`であり、KAI-15では予備試行前のUI接続・採点処理・再受験制御・統合検証へ進む。
...
- KAI-15の残作業: Quiz UIへのノード別接続、採点・許容解処理、再受験制御、予備試行前統合検証、3〜5名程度の予備試行。
...
- [確認済み事実] 新規担当者は、本パッケージだけで研究目的、MVP境界、確定事項、未確定事項、次に仕様化すべき論点を把握し、KAI-15のUI接続・採点処理・再受験制御、またはKAI-12/OQ-009の仕様検討を開始できる。コード変更を伴う作業開始には、最新リポジトリの取得と実装状態の再確認が必要である。
