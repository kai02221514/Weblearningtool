...
- 次の最小作業:
  1. `route-spec/1.0`準拠の純粋な`routeGenerator`実装と代表シナリオ検証を、KAI-15へ混在させず、独立Linear Issue・独立Draft PRで扱う。保存、評価ログ、`generatedAt`、`routeId`等のOQ-009依存事項は含めない。
  ...
  5. KAI-14、KAI-24、KAI-25はDoneでmain反映済みである。KAI-25の実装作業は次の最小作業から除外する。
...
[禁止] OQ-009の未確定事項、保存、評価ログを、Codexまたは他AIが実装上の既定値で補完してはならない。KAI-15の予備試行用3ノード9問に関する短いコード補完の許容解・正規化規則はD-020に従い、未記載の許容解を独自に追加しない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験・2形式限定・合格後再受験なしはD-018に基づく初期仕様だが、予備試行結果に基づいて見直し得る。Quiz UI接続はKAI-21で対象3ノードに限定して実装済みであり、KAI-23でメモリ内の再受験制御を追加済みである。純粋な`routeGenerator`実装は独立Issueで扱い、KAI-12未確定事項を含む保存・評価ログ層へ拡張しない。
...
- Issue分離: routeGenerator専用の独立Linear Issueを作成し、KAI-15へ混在させない。本タスクではLinear Issueを作成しない。
