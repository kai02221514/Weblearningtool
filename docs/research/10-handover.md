...
- 状態: Phase 2およびPhase 2.5正式完了。Phase 3のKAI-9/KAI-10判断はPR #1で`main`へ反映済み、Linear上もDone。KAI-11/OQ-006は仕様案作成段階であり、解消済みではない。
...
- Phase 3進行状況（2026-07-03）: KAI-9/KAI-10の研究者判断を`docs/architecture/route-generation.md`（`route-spec/1.0`、状態: 承認済み仕様）、`01-confirmed-decisions.md`、`02-open-questions.md`、`09-decision-log.md`へPR #1で反映済み。D-016でOQ-004は条件付き解消、D-017でOQ-005は解消した。KAI-11/OQ-006は`docs/architecture/quiz-assessment.md`に仕様案（`quiz-assessment-proposal/0.1`）を作成済みだが、研究者レビュー、指導教員確認、予備試行の前であり、OQ-006は未解消である。
...
- 未完了事項: OQ-006の正式採用判断、OQ-009、`routeGenerator`、ノード別確認テスト・教材、同意・永続化・評価ログ、テスト・型検査・CI基盤、P-06/P-07/P-09の保留判断。
- 指導教員確認が必要なIssue: `KAI-12`、`KAI-16`は必須。`KAI-11`、`KAI-15`は評価設計への影響により推奨。KAI-9/KAI-10で採用した開始2候補、上位3件提示、6段階優先順位を研究主張へ使う場合も確認推奨。

[禁止] OQ-006、OQ-009の未確定事項を、Codexまたは他AIが実装上の既定値で補完してはならない。`docs/architecture/quiz-assessment.md`の3問・2/3合格・無制限再受験は採用候補であり、研究者レビューと必要な確認なしに確定扱いしない。routeGenerator実装はまだ開始しない。
...
Phase 3では、以下の仕様確定Issueを優先する。`KAI-9`と`KAI-10`は研究者判断を`main`へ反映しLinear上も完了済みであり、次の最小作業は`KAI-11`である。

- `KAI-9`: OQ-004 診断規則と開始ノード規則を確定する。状態: 完了。DG-08のみOQ-009へ移管。
- `KAI-10`: OQ-005 ルート生成規則と出力契約を確定する。状態: 完了。
- `KAI-11`: OQ-006 確認テスト規則を確定する。状態: In Progress。
...
- 現在の仕様案: `docs/architecture/quiz-assessment.md`（`quiz-assessment-proposal/0.1`）
- KAI-11の範囲: 問題数、問題形式、採点、合格判定、再受験、合格後再受験、不合格時の主推薦、誤答と補助前提ノード、問題集合版、論理データ契約を決める。
- KAI-15の範囲: 実際の問題文、選択肢、正答、許容解、解説、問題ごとの関連前提ノード、教材本文との整合を整備する。
- 前提: 問題数、合格閾値、再受験規則が研究者レビューと必要な確認を経て確定していること。
...
- KAI-11仕様案の採用候補を、Decision Log追記前に確定事項として扱う。
