...
- KAI-29現在地: 連続録画用の学習導線UI整理を作業ブランチで実装し、監査指摘対応commit `b7509680b3eb1f5abb6cae415813ddd4560acaca`で状態作成後の再ログイン初期化テストを強化した。合成データ専用local Supabaseと通常アプリentryを接続し、K群3項目の保存、DG-RULE-3、教材→確認テスト→実践課題→振り返り、単元完了後の推薦更新、再ログイン時の診断だけの復元と学習状態0件化を実APIで完走した。local stack・開発サーバーは停止し、DB volume・一時環境ファイルは削除済みである。`manual/kai-29/index.html`はremoteへ接続しない合成データ専用ハーネスで、本番entryから参照しない。空DBのsignupにはmigration化されていない既存KVテーブルを一時作成する前提が残り、ブラウザの`clientWidth`直接値とTab移動は未取得である。学習状態の永続化、認証session自動復元、同意・評価ログ・研究データ出力は対象外で、新しい研究判断は追加していない
