...
- 予備試行用確認テストデータ: `html-010`、`html-021`、`css-011` の3ノード9問を `src/features/quiz/` 配下の型付きデータへ変換し、ID、版、形式、参照整合性の構造検証テストを追加した。これはUI接続、採点、再受験、保存、予備試行完了を意味しない。
...
- 自動テスト: [部分確認済み] MVPノード定義と予備試行用クイズカタログの構造検証テストを追加。テスト基盤全体、lint、CIはKAI-13として継続
...
2. KAI-15でQuiz UIへのノード別接続、採点・許容解処理、再受験制御、予備試行前統合検証、3〜5名程度の予備試行へ進む。
3. Phase 3でOQ-009を解消し、研究データ管理、同意、保存、削除、アクセス権限を確定する。
4. 上記の確定後に、テスト基盤、`routeGenerator`、永続化、評価ログ等の実装Issueへ進む。
