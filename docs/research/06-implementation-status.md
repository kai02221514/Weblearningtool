...
- KAI-29対象Issue / 実装PR: Linear KAI-29（Done、完了コメント登録済み）/ PR #36（merged、branch `feat/kai-29-learning-flow-ui`）。最終head `8aa559b8f07a4f201cdef6b8a7ace8498cc1dd18`を2026-09-06T08:18:07Zにmerge commit `db6f4dc017ca062db050469e07332a9977c06a4a`として`main`へ反映した
- 追加確認日: 2026-07-05
...
|現在の認証・データ関連|Supabaseを利用する認証処理コード・型付き表示名と診断の保存用エンドポイント|[部分実装／local・remote合成データ検証済み] KAI-34で指定remoteへprofilesとuser_diagnosesを適用し、本人・他人・未認証境界、再ログイン復元、失敗境界、cleanupを確認済み。参加者データ利用や研究上の有効性は未確認|
|現在のAPI関連|Hono依存関係およびSupabase Functions配下のサーバーコードあり|[部分実装／local・remote合成データ検証済み] KAI-34で監査済みFunctionをversion 5としてdeployし、legacy `/profile` 404と内部Bearer認証を確認済み。`verify_jwt=false`は維持|
...
- 表示名profile: [KAI-32・KAI-33でmain反映／KAI-34 remote合成検証済み] 型付き`public.profiles.display_name`を正本とするsignup/signin/read/update、本人限定アクセス、Auth user削除連動を実装した。KAI-34で指定remoteへ適用し、本人成功、他人拒否、未認証拒否、再ログイン復元、profile失敗境界、cleanupを確認した。5項目は採用・移行していない。
...
- Edge Function: 旧 `src/supabase/functions/server` 配置から `supabase/functions/make-server-f3d88633` へ移行した。KAI-28 / PR #34でBearer tokenを`auth.getUser`により明示検証し、user-scoped clientでRLSを通す`GET /diagnosis`と`PUT /diagnosis`を追加した。KAI-33 / PR #45でlegacy `/profile`とservice-role KV helperを撤去し、KAI-34で指定remoteへversion 5 / artifact SHA-256 `caec2a968e3899ce06fb482af6f43bd824ec93877afac574c5c2213395be7c77`としてdeployした。`verify_jwt=false`を維持し、保護APIの内部Bearer認証境界を合成データで確認済みである
...
- コード状態の最新対象コミット: `db6f4dc017ca062db050469e07332a9977c06a4a`（KAI-29 / PR #36のmerge commit、main反映・確認済み）
...
- KAI-29学習導線UI整理: [main反映・確認済み] PR #36の最終head `8aa559b8f07a4f201cdef6b8a7ace8498cc1dd18`をmerge commit `db6f4dc017ca062db050469e07332a9977c06a4a`として`main`へ反映した。ログイン・アカウント作成の状態表示、K群3項目だけの必須診断、推薦を主導線にしたDashboard、教材・確認テスト・実践課題・振り返りの共通4段階表示、振り返り確定後の明示的な単元完了、ログイン画面へ戻る導線と保存済み診断からの推薦再生成表示を接続した。固定の週次目標、連続日数、総学習時間、実践課題の固定Lesson表示、保存・最適化・学習効果を示唆する未接続文言は表示対象から除外した
- KAI-29監査指摘対応の自動検証: [main反映・確認済み] 2026-09-06のcommit `b7509680b3eb1f5abb6cae415813ddd4560acaca`で、保存済みK群診断から`html-010`を開始し、3問クイズ、実践課題、振り返り、単元完了、`html-020`開始まで実際に状態を作るApp UI統合テストへ強化した。ログイン画面へ戻った後の同一合成利用者の再ログインで、診断再取得とDG-RULE-3による`html-010`再生成、初回診断の非表示、`completedNodeIds`、クイズ履歴、振り返り、`inProgressNodeId`の空状態、セッション進捗・テスト回数0、`unit-completed`通知の消去と`diagnosis-restored`通知を安定した識別子で確認した。対象UI 2 files / 10 tests、routeGenerator・quiz・practice 9 files / 142 tests、`npm run verify`（typecheck、lint、全19 files / 213 tests、build 1725 modules transformed）、`git diff --check`に成功し、通常production buildに`manual/kai-29/`固有entry・制御文言がないことも確認した
- KAI-29第2回監査指摘対応: [main反映・確認済み] commit `3a07eb9f8c90c3e94169bde047ed6508ff4a2373`で、再ログイン後に再生成された`html-010`から確認テストへ進み、以前の合格結果と合格済み導線がなく、問題1 / 3、未回答の選択肢4件、`試行 1 / 進捗: 33%`から開始することを直接検証した。新しい全問正解後も結果識別子が`quiz-html-010 / quiz-html-010/v0.2 / 試行1`となる。`setQuizAttemptHistory([])`だけを一時的に除去する変異確認では対象テストが期待どおり失敗し、復元後は対象UI 2 files / 10 tests、routeGenerator・quiz・practice 9 files / 142 tests、`npm run verify`（typecheck、lint、全19 files / 213 tests、build 1725 modules transformed）、`git diff --check`、production build境界確認に成功した。空DBで通常entryのsignupが前提とするKVテーブルは実装せず、調査Issue [KAI-30](https://linear.app/kai02221514/issue/KAI-30/)をBacklogに作成してKAI-29・KAI-12へ関連付けた
- KAI-29 local Supabase実APIブラウザ検証: [main反映・確認済み] remote linkのない専用一時projectを`127.0.0.1:55321`へ起動し、空DBから診断migrationを再構築した。pgTAP 1 file / 24 tests、DB lint schema error 0件、合成利用者A/Bの診断API統合に成功した。通常アプリentryで合成`.test`アカウントを作成し、ログイン、K群3項目だけの初回診断、未回答時のDashboard遷移拒否と`role=alert`、`yes / confident / structure_style`保存、DG-RULE-3、教材、3問全問正解、実践課題の限定自動判定と表示確認、振り返り、単元完了、推薦更新、`html-020`途中状態作成、ログイン画面復帰、同一利用者の再ログイン、診断だけの復元、完了・クイズ・振り返り・進行中状態の0件化を一続きに確認した。1920×1080は`scrollWidth=1920 / innerWidth=1920`、390×844は`scrollWidth=390 / innerWidth=390`で横スクロールなしだった。可視outline、教材の右矢印とEnter、読み上げ可能な保存エラー、ログイン送信中のdisabledを確認し、console warning/errorは0件だった。開発サーバーとlocal Supabaseを停止し、合成DB volumeと`.env.local`を削除した
...
- KAI-30 KV依存調査: [調査完了・未実装] PR #38の最終head `0f9ac201c4cc5b33f96c80b273a1f445d6633328`をmerge commit `9c90855010a8a430bd5713fcb398f47714244bd3`としてmainへ反映し、main `Check` run [34024031887](https://github.com/kai02221514/Weblearningtool/actions/runs/34024031887)が成功した。現行signupがAuth user作成後にmigration未管理のKV tableへ必須書込みしてfresh環境でHTTP 500となること、signinがKV nameを優先しAuth metadataへfallbackするため表示名の正本が重複すること、localと指定remote候補のschemaが異なることを確認した。KAI-30完了時点では、Linear KAI-30をDoneとし、後続Decision Issue [KAI-31](https://linear.app/kai02221514/issue/KAI-31/)をBacklogで作成していた。当時は表示名の正本・型契約・KV廃止範囲とproject ref `znfwkrhquegvlcmugkoe`の用途・deploy許可が未決定で、schema、migration、Edge Function、frontend、remote Supabaseへ未適用だった
- KAI-31研究者判断: [仕様確定・main反映・完了証跡登録済み・未実装・remote未適用] D-023により、表示名を研究分析に使わない通常運用データ、型付き`public.profiles.display_name`を唯一の正本、emailをSupabase Authの正本とした。trim後1〜50文字、空文字・改行・制御文字拒否、Unicode・本人変更許可、Auth user削除連動、DB時刻、signup完了境界、KV廃止方針を確定した。project ref `znfwkrhquegvlcmugkoe`は合成データ専用の非本番remote検証環境とし、監査済み・明示許可済み変更だけdeploy可能とした。研究者本人はD-023とその確定契約に基づくKAI-32〜KAI-35の着手判断を指導教員から委任されており、追加の指導教員承認を開始条件としない。これはD-023の技術内容の個別承認やremote変更の包括許可を意味せず、変更ごとの研究者本人の明示許可は必須である。文書PR [#40](https://github.com/kai02221514/Weblearningtool/pull/40)のfinal head `6b36d3c03174145c42ccdec6c443db7209186a0d`をmerge commit `83dc8d4e08d29b67a249022d678e98c8b3719b58`としてmainへ反映し、Linear KAI-31へ完了証跡を登録してDoneとした。PR headでは`Check` run [34026445041](https://github.com/kai02221514/Weblearningtool/actions/runs/34026445041)と`Supabase Diagnosis` run [34026445044](https://github.com/kai02221514/Weblearningtool/actions/runs/34026445044)が成功した。mainでは同merge commitを対象とする`Check` run [34026973738](https://github.com/kai02221514/Weblearningtool/actions/runs/34026973738)の全stepが成功し、main向け`Supabase Diagnosis`は未起動である。現行コード、schema、migration、RLS、GRANT、Edge Function、frontend、remoteは変更しておらず、profilesとKV廃止は未実装、remoteは未適用である。後続はKAI-32〜KAI-35へ分離した
- KAI-32 local実装: [完了／remote未適用] PR [#42](https://github.com/kai02221514/Weblearningtool/pull/42)の最終head `525d969866e12892cfdec2997b7833c1edf20e45`を、base `7906f32778251b13cff0a6a6c6560209ec55cb37`から2026-09-06T12:42:45Zにmerge commit `9b73e4010da4dc7d6e8c5305696a08fb9a015ff8`として`main`へ反映した。PR段階の最終merge-ref `153eb01b0ca83ff4e585fe9ace19fc9e3db17830`をcheckoutした`Check` run [34032277250](https://github.com/kai02221514/Weblearningtool/actions/runs/34032277250)と`Supabase Diagnosis` run [34032277253](https://github.com/kai02221514/Weblearningtool/actions/runs/34032277253)は全step成功だった。同merge commitのclean worktreeで`npm ci`、`npm run verify`の全20 files / 232 tests、build 1726 modules、`git diff --check`に成功した。別ポートの合成データ専用local projectでは、既定DBポート競合による初回`supabase start`失敗と、未使用Storageの再起動health check 502による最初のDB reset失敗を成功扱いせず記録し、既存projectを停止せずStorageを一時除外してfresh reset、pgTAP 2 files / 94 tests、DB lint、診断/profile API統合を再実行して成功し、使用したlocal projectだけを停止した。merge commitを対象・checkoutしたmain `Check` run [34033921358](https://github.com/kai02221514/Weblearningtool/actions/runs/34033921358)と、同じmain refで手動実行した`Supabase Diagnosis` run [34034224612](https://github.com/kai02221514/Weblearningtool/actions/runs/34034224612)も全step成功だった。完了証跡PR [#43](https://github.com/kai02221514/Weblearningtool/pull/43)の最終head `9a3f93380347a93a54713d9535f88652ee801843`を2026-09-06T13:07:28Zにmerge commit `9fb625a71177c339a1c085a34de1345edca429a0`としてmainへ反映し、同SHAのmain `Check` run [34035137530](https://github.com/kai02221514/Weblearningtool/actions/runs/34035137530)も全step成功した。remote Supabaseへの接続・変更、remote secret、実在個人情報、研究参加者データの使用はない。legacy `profile:{id}`全面撤去、KV廃止、remote schema差はKAI-33へ残し、KAI-33を開始可能とする
...
- 状態: [完了／remote適用はKAI-34で実施済み] 開始基準main `68f50a2784073a09c7733fc9171a94dc942da597`からbranch `refactor/kai-33-remove-legacy-kv`を作成し、PR [#45](https://github.com/kai02221514/Weblearningtool/pull/45)のfinal head `aaec41177e35cd30190b8b57ed65c4e4aafe9510`を2026-09-06T14:46:25Zにmerge commit `f7bf8c86ebae8e23c7c8ddb9aa9fbb43bf8b1246`として`main`へ反映した。完了証跡PR #46はmerge commit `22ca66d662067b639c33912e41bab3a81ee78c2d`として反映され、Linear KAI-33はDoneである
