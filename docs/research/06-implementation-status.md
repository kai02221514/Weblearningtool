...
- 型付きクイズカタログとUI非依存の採点・正規化関数が存在しても、現行Quiz UIはまだ固定問題を表示しており、`quiz-{nodeId}`を表示・保存しているわけではない。
...
2. KAI-21でQuiz UIをノード別クイズデータとKAI-22の採点関数へ接続する。
3. KAI-23で再受験制御を実装する。
4. KAI-12確定後に、必要な保存処理へ進む。
5. KAI-24で予備試行前統合検証を行い、その後3〜5名程度の予備試行へ進む。
6. Phase 3でOQ-009を解消し、研究データ管理、同意、保存、削除、アクセス権限を確定する。
