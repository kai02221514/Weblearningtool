...
- KAI-32 local実装: [Draft PR監査待ち・remote未適用] Draft PR [#42](https://github.com/kai02221514/Weblearningtool/pull/42)で、`public.profiles`、Auth user作成trigger、本人限定RLSと列単位GRANT、共有表示名validator、signup/signin/read/update API、frontendの`displayName`契約を実装した。Auth userとprofileは同一DB transactionで作成し、bootstrap用`user_metadata.display_name`は同じtransaction内で削除する。signinは利用者JWTとRLSでprofilesだけを読み、`user:{id}` KVおよびAuth metadataへfallbackしない。fresh local DB reset、pgTAP 2 files / 71 tests、DB lint、合成利用者profile/診断API統合、対象限定3 files / 22 tests、`npm run verify`の全20 files / 227 tests、build 1726 modules、`git diff --check`に成功した。ブラウザでは表示名文言、required、50文字上限、空白のみ拒否を確認した。remote Supabase、実在個人情報、研究参加者データは使用していない。CI、監査、merge、main再検証は未実施であり、Linear KAI-32はDoneにしていない
