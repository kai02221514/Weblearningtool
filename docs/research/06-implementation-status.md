...
- KAI-32 local実装: [監査指摘対応済み・再監査待ち・remote未適用] Draft PR [#42](https://github.com/kai02221514/Weblearningtool/pull/42)で、`public.profiles`、Auth user作成trigger、本人限定RLSと列単位GRANT、共有表示名validator、signup/signin/read/update API、frontendの`displayName`契約を実装した。初回head `bb2d598057c96566fec0bfb3ffdf3ebb395a2962`の`Check` run `34030858409`と`Supabase Diagnosis` run `34030858406`は成功したが、2026-09-06監査でDBの`btrim`とJavaScript `trim()`のUnicode空白差、および`service_role`権限のdefault privileges依存が要修正となった。commit `1aac38fd240a4842094fe8d6def0556123ae3db9`でECMAScriptのtrim対象集合をDB制約へ明示し、NBSP・U+3000・U+FEFFのData API直接保存拒否とEdge API正規化、拒否後signin成功を追加検証した。table権限は全roleをrevoke後、`authenticated`へ既存列権限、`service_role`へSELECT/DELETEだけをgrantし、3 trigger関数の直接EXECUTE拒否を明示した。fresh reset、pgTAP 2 files / 94 tests、DB lint、profile/診断API統合、対象validator 18 tests、`npm run verify`の全20 files / 232 tests、build 1726 modules、`git diff --check`に成功し、同commitの`Check` run `34032015077`と`Supabase Diagnosis` run `34032015105`も成功した。Auth/profile原子性、metadata除去、KV fallback不在を維持し、remote Supabase、実在個人情報、研究参加者データは使用していない。merge・main再検証は未実施で、Linear KAI-32はIn Progressのままである
...
- D-023表示名profile: [local実装・検証済み／remote未適用] KAI-32 / Draft PR #42でsignup/signin/read/update、本人限定アクセス、Auth削除連動を合成データで確認した。legacy `profile:{id}` 5項目APIの全面撤去はKAI-33へ残す
...
2. KAI-32 Draft PR #42のlocal実装・検証証跡を監査し、main反映可否を判断する。remote変更を混在させない。
3. KAI-32をmainへ反映・再検証した後にKAI-33でKV廃止・remote差分を整理する。KAI-33の停止条件を満たした後だけ、KAI-34で指定remoteへ監査済み変更を適用する。
