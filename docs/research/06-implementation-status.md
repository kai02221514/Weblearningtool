...
1. Phase 3でOQ-004を解消し、診断規則、開始ノード規則、欠損時規則を確定する。
2. Phase 3でOQ-005を解消し、ルート生成判断規則と出力契約を確定する。
3. Phase 3でOQ-006を解消し、確認テスト規則と教材整備範囲を確定する。
4. Phase 3でOQ-009を解消し、研究データ管理、同意、保存、削除、アクセス権限を確定する。
5. 上記の確定後に、テスト基盤、`routeGenerator`、永続化、評価ログ等の実装Issueへ進む。
