...
- 予備試行用確認テストデータ: `html-010`、`html-021`、`css-011` の3ノード9問を `src/features/quiz/` 配下の型付きデータへ変換し、ID、版、形式、出典参照、参照整合性の構造検証テストを追加した。PR #6の範囲は型付きデータと構造検証に限定され、UI接続、採点、許容解正規化、再受験、保存、予備試行完了を意味しない。
...
- Quiz UIへのノード別クイズデータ接続
- 確認テストの採点実装
- 許容解正規化処理
- 未確定の許容解候補を採点対象へ含めるかの研究者判断
- 再受験処理・試行履歴管理
...
- 型付きクイズカタログが存在しても、現行Quiz UIはまだ固定問題を表示しており、`quiz-{nodeId}`を表示・採点・保存しているわけではない。
...
2. KAI-15で未確定の許容解候補と正規化規則について研究者判断を得る。
3. KAI-22でUIから独立した採点処理を実装し、確定済みの許容解だけを扱う。
4. KAI-21でQuiz UIをノード別クイズデータへ接続する。
5. KAI-23で再受験制御を実装する。
6. KAI-12確定後に、必要な保存処理へ進む。
7. KAI-24で予備試行前統合検証を行い、その後3〜5名程度の予備試行へ進む。
8. Phase 3でOQ-009を解消し、研究データ管理、同意、保存、削除、アクセス権限を確定する。
