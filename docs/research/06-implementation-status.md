...
2. KAI-32完了証跡文書PRを監査し、main反映後の`Check`成功とLinear KAI-32の完了コメント・Doneを確認する。remote変更を混在させない。
3. KAI-32完全完了ゲート成立後にKAI-33でKV廃止・remote差分をread-only調査し、KVが1行でも存在する場合は値を取得せず停止する。KAI-33の停止条件を満たした後だけ、KAI-34で指定remoteへ監査済み変更を適用する。
