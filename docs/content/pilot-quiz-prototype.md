...
- 対象仕様: `docs/architecture/quiz-assessment.md`（`quiz-assessment-proposal/0.2`、研究者レビュー済み仕様案）、`docs/architecture/route-generation.md`（`route-spec/1.0`）
- 対象ノード: `html-010`、`html-021`、`css-011`（quiz-assessment.md §30.1の予備試行対象案に一致）
- 問題数: 各ノード3問、合計9問
- 状態: [試作・研究者レビュー前] 全9問が「要研究者確認」である。予備試行未実施。UI実装・問題データ化は未着手。
...
- 教材根拠: `docs/content/pilot-material-draft.md`（教材本文草案。本試作と同時に作成。研究者レビュー前）

[重要な前提] 2026-07-04時点で対象3ノードのノード別教材本文はリポジトリに存在しなかった（詳細は`docs/content/pilot-material-draft.md` §1）。本試作の全問題は、同時作成した教材本文草案を参照基準とする。教材草案が未承認である以上、本試作の「教材範囲との整合」は草案基準の暫定的なものであり、教材承認後に再確認が必要である。
...
以下は指導教員承認前だが研究者レビュー済みの仕様案（quiz-assessment-proposal/0.2）であり、本試作はこれに従う。
...
[注意] 3問・2/3合格・2形式限定・無制限再受験は、文献が直接決めた値ではなくMVPと予備試行のための研究者設計判断であり、予備試行後に再確認される（§10、§13、QA-03/QA-05/QA-07）。
...
| `docs/research/02-open-questions.md` | 全文 | OQ-006の未解消状態、KAI-15移管範囲、予備試行対象3ノード |
...
| `docs/research/09-decision-log.md` | 全文 | D-001〜D-017、D-018が未追加であること |
...
- 文献上の一般原則: 形成的評価として設計する、解説は誤概念修正を含める、誤答選択肢はもっともらしい誤概念を表す、問題は到達目標と対応させる、初学者の認知負荷を抑える。これらは§3.3の文献が支える一般方針であり、個々の問題内容を決めるものではない。
- プロジェクト資料上の要件: 3問構成、2形式、1点×3、2/3合格、`relatedPrerequisiteNodeIds`、版管理、解説必須項目。これらはquiz-assessment-proposal/0.2の研究者レビュー済み仕様案に従う。
...
| html-010-q3 | head要素の役割・記述 | コード補完 | 骨格内の該当位置に入る要素名（head）の産出 | q1の役割理解を産出形式で再確認 | 中 |
...
- sourceReferences:
  ...
  academic:
    - title: A Review of Multiple-Choice Item-Writing Guidelines for Classroom Assessment
      ...
      usedFor: 誤答選択肢をもっともらしい誤概念で構成し、選択肢の粒度（すべてタグ表記）をそろえる設計
...
- sourceReferences:
  ...
  academic:
    - title: A Review of Multiple-Choice Item-Writing Guidelines for Classroom Assessment
      ...
      doiOrUrl: https://doi.org/10.1207/S15324818AME1503_5
      verificationLevel: 書誌情報・要旨のみ確認
      usedFor: 選択肢を同形式・同分量のコードでそろえ、正答だけが目立たない設計
      limitation: この問題の正答や選択肢数を直接決めるものではない
    - title: Cognitive Load During Problem Solving
      ...
      usedFor: 各選択肢のコードを空要素表記で最短化し、読解負荷を抑える判断
...
- targetObjective: 最小HTML文書の骨格内で、titleを含む「ページに関する情報」を置く要素（head）を産出できる（quiz-assessment.md §9「役割と配置」の役割の産出的確認）
- observableBehavior: 骨格コードの空欄に入る要素名として「head」を入力できる。
- prompt:

  次の最小HTML文書の2つの空欄には、同じ要素名が入ります。その要素名を答えてください（例: body）。
...
- acceptedAnswers:
  - 正答: `head`
  - 正規化（案・§12参照）: 前後空白除去、小文字化して比較（`HEAD`、`Head`も正解扱い）。
  - 許容解に含めるか要確定: `<head>`（山括弧付き回答）。問題文の「要素名を答えてください（例: body）」で括弧なしを誘導するが、括弧付き回答を不正解にすると採点漏れの恐れがある。KAI-15で確定が必要。
  - 不正解とする回答例: `header`（heade要素は未学習だが、混同回答として記録価値がある）、`title`、`html`。
  - 正答の一意性: 教材草案の骨格において、titleを直接含みbodyと並ぶ要素はheadのみであり、一意である。
- explanation:

  正解は`head`です。`<title>`のような「ページに関する情報」は`<head>`〜`</head>`の中に書きます。`<head>`の中身は本文としては表示されず、`<title>`の文字はブラウザのタブなどに表示されます。画面に表示したい内容（`<p>こんにちは</p>`など）は`<body>`に書きます。`body`や`html`と答えた場合は、それぞれの要素の役割を混同しています。復習する場合は教材「最小のHTML文書」「head要素の役割」（pilot-material-draft.md §3.1、§3.4）を読み直してください。

- misconceptionByChoice: （選択式でないため、想定誤答と対応する誤概念）
  - `body`: head/bodyの役割混同（q1と同じ誤概念の産出形式での再出現）。
  - `title`: 「titleを包むのはtitle系の何か」という表層的推測。要素の包含関係の未理解。
...
- sourceReferences:
  ...
  material:
    - path: docs/content/pilot-material-draft.md
      ...
      usedFor: 骨格コードと正答。§3.1のコード例から題材を作成
  academic:
    - title: Test-Enhanced Learning
      ...
      verificationLevel: 書誌情報・要旨確認（quiz-assessment.md §27引き継ぎ）
      usedFor: 再認（選択式）だけでなく想起・産出を1問含める構成の背景
...
- designDecision: 空欄は開始タグと終了タグの2箇所だが、同一の要素名1語を1つの解答欄で回答する設計とし、論理的な補完箇所を1つに保つ（「補完箇所は原則1か所」の运用としてこの解釈を採る。要研究者確認）。終了タグ側を表示したまま開始タグのみ空欄にすると答えが転記できてしまうため、この形式を選んだ。回答を記号なしの1語に限定したのは採点の機械化と採点漏れ防止のため。例示に`body`を使ったのは、回答形式（括弧なし）を示しつつ、コード中で既に使われており答えのヒントにならないため。
- ambiguityCheck: 正答は一意（上記）。「同じ要素名が入ります」の指示が初学者に伝わるかは予備試行で確認する。
- outOfScopeCheck: 教材§3.1のコード例とほぼ同一の骨格を使用。未学習要素なし。
- pilotReviewPoints: 無回答率（産出問題は選択式より無回答が出やすい）。`<head>`形式（括弧付き）の回答の発生率（許容解仕様の確定材料）。大文字回答の発生率。所要時間。
...
`docs/content/pilot-material-draft.md` §4（html-021: 入れ子構造）。扱う内容: 入れ子と親要素・子要素（§4.1）、閉じる順序（§4.2）、交差した入れ子の誤り（§4.3）、インデント（§4.4）、よくある間違い（§4.5）。扱わない内容: DOM API、JavaScriptによるDOM操作、パーサーのエラー回復、要素固有の入れ子制約（コンテンツモデル）、兄弟関係の詳細。使用要素はp、strong、body（教材§4の登場要素）に限定する。
...
- type: 単一選択（4択・コード提示）
- difficulty: 基礎
  - 根拠: 3要素のみの短いコードで、必要知識は「親要素＝直接包む1つ外側の要素」の定義1つ。ただし選択肢2（body・祖先）との識別に定義の正確な理解が必要なため、識別難度は完全な初歩よりわずかに高い。総合して基礎とする。
- targetObjective: 入れ子構造における親子関係の判別（quiz-assessment.md §9「親子関係…を判別できる」）
- observableBehavior: 提示コード中の`<strong>`について、直接の親要素`<p>`を、祖先（body）・自要素・テキストと区別して選択できる。
- prompt:
...
- explanation:
  ...
  正解は`<p>`です。親要素とは、その要素を直接包んでいる1つ外側の要素のことです。`<strong>`を直接包んでいるのは`<p>`です。`<body>`は`<p>`のさらに外側にあるので、`<strong>`から見ると親のさらに外側（祖先）であり、直接の親ではありません。`<strong>`自身は親にはなれません。「重要」は`<strong>`の中身（テキスト）であり、要素ではありません。復習する場合は教材「要素の中に要素を入れる（入れ子）」（pilot-material-draft.md §4.1）を読み直してください。

- misconceptionByChoice:
  - 選択肢2 `<body>`: 直接の親と祖先の区別ができていない（「包んでいる要素なら何でも親」という誤概念）。可能性: 高。本問の中心的識別対象。
  - 選択肢3 `<strong>`: 親子概念そのものが未成立（問われている関係を自要素へ帰属）。可能性: 低。
  - 選択肢4 テキスト: 要素とその内容（テキスト）の区別ができていない。可能性: 低〜中。
...
- sourceReferences:
  ...
  academic:
    - title: A Review of Multiple-Choice Item-Writing Guidelines for Classroom Assessment
      ...
      doiOrUrl: https://doi.org/10.1207/S15324818AME1503_5
      verificationLevel: 書誌情報・要旨のみ確認
      usedFor: 中心的な識別対象（親vs祖先）をもっともらしい誤答として置く設計
      limitation: この問題の正答や選択肢数を直接決めるものではない
  ```

- educationalRationale: 親子関係は復習推薦（誤答→前提ノード）や後続ノード（html-031、html-040、css-021）の基盤となる概念であり、定義の正確な理解（直接性）を確認する。誤概念「祖先も親」は選択肢2で直接検出できる。
- designDecision: 問題文に親要素の定義（「直接包んでいる、1つ外側の要素」）を再掲した。用語の記憶ではなく構造の読み取りを測るためであり、定義自体を問う問題にしない判断。選択肢4のみ粒度が異なる（タグでなくテキスト）が、「要素と内容の混同」という実在の誤概念を表すため採用した（要研究者確認）。
- ambiguityCheck: 正答は一意。定義を問題文に含めたため解釈のぶれは小さい。選択肢4の表記粒度の違いが推測の手がかりにならないか予備試行で確認する。
- outOfScopeCheck: 使用要素はbody・p・strong（教材§4の登場要素）のみ。「祖先」という用語は選択肢・解説の説明内でのみ使用し、正答の判断には不要。DOM APIやツリー用語（ノード、ルート等）は不使用。
- pilotReviewPoints: 選択肢2（body）の選択率（親と祖先の区別の定着度）。問題文中の定義再掲で十分に意味が伝わるか（分かりやすさ評価）。
...
- sourceReferences:
  ...
  academic:
    - title: A Review of Multiple-Choice Item-Writing Guidelines for Classroom Assessment
      ...
      doiOrUrl: https://doi.org/10.1207/S15324818AME1503_5
      verificationLevel: 書誌情報・要旨のみ確認
      usedFor: 全選択肢を同形式・同分量の1行コードでそろえる設計
      limitation: この問題の正答や選択肢数を直接決めるものではない
  ```
...
- sourceReferences:
  ...
  academic:
    - title: Test-Enhanced Learning
      ...
      doiOrUrl: https://doi.org/10.1111/j.1467-9280.2006.01693.x
      verificationLevel: 書誌情報・要旨確認（quiz-assessment.md §27引き継ぎ）
      usedFor: 閉じ順を再認（q2）と産出（本問）の両形式で確認する構成の背景
      limitation: 本問の学習効果を実証するものではない
  ```
...
| css-011-q3 | 宣言の産出 | コード補完 | 目的（文字色変更）に対応するプロパティ名の産出 | q1の役割理解を産出形式で確認 | 中 |
...
- sourceReferences:
  ...
  academic:
    - title: A Review of Multiple-Choice Item-Writing Guidelines for Classroom Assessment
      ...
      doiOrUrl: https://doi.org/10.1207/S15324818AME1503_5
      verificationLevel: 書誌情報・要旨のみ確認
      usedFor: 選択肢を同一コード内の構成部位でそろえ、外部知識を要求しない設計
      limitation: この問題の正答や選択肢数を直接決めるものではない
  ```
...
- targetObjective: 目的に対応するプロパティの産出（quiz-assessment.md §9「プロパティ…の役割を判別できる」の産出的確認。ノード定義summary「基本形と; {} の意味を理解して書ける」の「書ける」に対応）
...
- acceptedAnswers:
  - 正答: `color`
  - 正規化（案・§12参照): 前後空白除去、小文字化して比較（CSSのプロパティ名は大文字小文字を区別しないため`COLOR`等も正解扱い）。
...
- sourceReferences:
  ...
  academic:
    - title: Test-Enhanced Learning
      ...
      doiOrUrl: https://doi.org/10.1111/j.1467-9280.2006.01693.x
      verificationLevel: 書誌情報・要旨確認（quiz-assessment.md §27引き継ぎ）
      usedFor: 再認2問に産出1問を組み合わせる構成の背景
      limitation: 本問の学習効果を実証するものではない
  ```

- educationalRationale: ノード定義の「書ける」に対応する産出問題。プロパティ名が決められた語彙であること（推測で作れないこと）の確認は、実践課題でのCSS記述に直結する。
...
| html-010-q3 | html-010 | 役割の産出 | コード補完 | 標準 | 骨格内のhead産出 | q1と役割理解を共有 | 中 |
...
| css-011-q3 | css-011 | 宣言の産出 | コード補完 | 標準 | プロパティ名の産出 | q1と部位役割を共有 | 中 |
...
- コード補完が文字暗記にならないかの確認: html-010-q3は骨格内の位置から役割を判断する必要があり、html-021-q3は構造から閉じ順を判断する必要がある。css-011-q3はプロパティ語彙の想起要素が最も強く、暗記寄りである（§10の監査で明記）。
...
| html-010-q3 | ○ | ○（草案基準） | ○ | ○ | ○ | ○ | ○ | 要研究者確認（空欄2箇所1解答の形式） |
| html-021-q1 | ○ | ○（草案基準） | ○ | ○ | ○ | ○ | ○ | 要研究者確認（選択肢4の粒度差） |
| html-021-q2 | ○ | ○（草案基準） | ○ | ○ | ○ | ○ | ○ | 要研究者確認 |
| html-021-q3 | ○ | ○（草案基準） | ○ | ○ | ○ | ○ | ○ | 要研究者確認 |
| css-011-q1 | ○ | ○（草案基準） | ○ | ○ | ○ | ○ | ○ | 要研究者確認 |
| css-011-q2 | ○ | ○（草案基準） | ○ | ○ | ○ | ○ | ○ | 要研究者確認 |
| css-011-q3 | ○ | ○（草案基準） | ○（許容解集合基準） | ○ | ○ | ○ | ○ | 要研究者確認（綴り誤りの扱い、暗記寄り） |

[重要] 「教材範囲内○」はすべて未承認の教材草案（pilot-material-draft.md）基準である。教材草案が承認・修正されるまで、全9問を「要研究者確認」とし、完成扱いにしない。

追加確認項目:

- 正答が問題文から推測できないか: 各問題の例示語（body、font-size等）が正答と重複しないことを確認した。html-010-q3で終了タグを非表示にし転記を防いだ。
- 正答だけ表現が異ならないか・選択肢の長さの偏り: 全単一選択で選択肢を同形式（タグ表記、1行コード、部位表記）にそろえた。例外はhtml-021-q1選択肢4（テキスト）で、粒度差があるため要研究者確認とした。
...
3. html-010-q3の「空欄2箇所・同一語1解答」形式が「補完箇所は原則1か所」の運用として妥当か。
4. css-011-q3で綴り誤り（colour等）を不正解とする扱い。
5. html-021-q1選択肢4（テキスト選択肢）の粒度差の許容。
...
9. OQ-006自体が未解消であること（指導教員確認・予備試行・D-018追加の前）。本試作の9問は、OQ-006の前提（3問・2/3合格・2形式）が変更された場合に再設計が必要になる。
