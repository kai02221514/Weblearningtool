...
const challenge = {
  title: "自己紹介ページを作成しよう",
  description:
    "HTMLとCSSを使って、あなた自身の自己紹介ページを作成してください。",
  requirements: [
    "h1タグで名前を表示",
    "pタグで自己紹介文を記述",
    "h2タグで「趣味」の見出しを追加",
...
export function PracticeChallenge({
  ...
  const [selectedNodeForPreview, setSelectedNodeForPreview] = useState<
    string | null
  >(null);

  // 現在の課題ノード（実際のアプリではpropsで渡すべきだが、ここではデモ用にhtml-010を使用）
  const currentChallengeNodeId = "html-010"; // 例: HTMLリスト基礎の課題
  const currentNode = learningNodesArray.find(
    (n) => n.id === currentChallengeNodeId
  );
  ...
  const analyzeCode = (currentCode: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      openTags.forEach((tag) => {
        ...
        if (
          ...
          if (!currentCode.includes(closeTag)) {
            ...
            const errorMapping = errorMappingsArray.find(
              (e: any) => e.id === "E_HTML_MISSING_CLOSING_TAG"
            );
            const relatedNodeIds =
              errorMapping?.nodeRefs?.map((ref: any) => ref.nodeId) || [];
            skills.push({
              line: index + 1,
      ...
      if (
        ...
        const inStyle = currentCode
          .substring(0, currentCode.indexOf(line))
          .includes("<style>");
        if (inStyle) {
          // E_CSS_SYNTAX_MISSING_SEMICOLON に対応
          const errorMapping = errorMappingsArray.find(
            (e: any) => e.id === "E_CSS_SYNTAX_MISSING_SEMICOLON"
          );
          const relatedNodeIds =
            errorMapping?.nodeRefs?.map((ref: any) => ref.nodeId) || [];
          skills.push({
    ...
    if (ulMatch) {
      ulMatch.forEach((ul) => {
        ...
        if (content.match(/<(?!li|\/li)[a-z]/i)) {
          ...
          const errorMapping = errorMappingsArray.find(
            (e: any) => e.id === "E_HTML_INVALID_NESTING"
          );
          const relatedNodeIds =
            errorMapping?.nodeRefs?.map((ref: any) => ref.nodeId) || [];
          rules.push({
            message:
              "`<ul>` の直下には `<li>` だけを入れます。現在は他の要素が入っています。",
            example: "<ul>\n  <li>項目1</li>\n  <li>項目2</li>\n</ul>",
            relatedNodeIds,
          });
        }
    ...
    if (!currentCode.includes("<!DOCTYPE html>")) {
      rules.push({
        message:
          "`<!DOCTYPE html>` 宣言がありません。HTMLファイルの先頭に追加してください。",
      });
    }
    ...
    if (h1Count > 1) {
      ...
      const errorMapping = errorMappingsArray.find(
        (e: any) => e.id === "E_HTML_HEADING_STRUCTURE"
      );
      const relatedNodeIds =
        errorMapping?.nodeRefs?.map((ref: any) => ref.nodeId) || [];
      knowledge.push({
    ...
    if (hasH3 && !hasH1) {
      ...
      knowledge.push({
        message:
          "h3タグを使う前に、h1タグから順番に使用することが推奨されます。",
        relatedNodeIds,
      });
    }
    ...
    if (imgWithoutAlt && imgWithoutAlt.length > 0) {
      ...
      const errorMapping = errorMappingsArray.find(
        (e: any) => e.id === "E_HTML_MISSING_REQUIRED_ATTR"
      );
      const relatedNodeIds =
        errorMapping?.nodeRefs?.map((ref: any) => ref.nodeId) || [];
      knowledge.push({
        message:
          "画像タグ（img）には、アクセシビリティのためalt属性を追加することが推奨されます。",
        relatedNodeIds,
      });
    }
  ...
  const getRecommendationsFromErrors = () => {
    ...
    // エラーマッピングから推奨ノードを取得
    const recommendations = detectedErrorIds
      .map((errorId) => {
        const mapping = errorMappingsArray.find((m: any) => m.id === errorId);
        return mapping;
      })
      .filter(Boolean);
    ...
    recommendations.sort((a, b) => {
      ...
      const aScore =
        (srkOrder[a!.srk as keyof typeof srkOrder] || 0) * 10 +
        (severityOrder[a!.severity as keyof typeof severityOrder] || 0);
      const bScore =
        (srkOrder[b!.srk as keyof typeof srkOrder] || 0) * 10 +
  ...
  const recommendNodeIds = Array.from(
    new Set(recommendations.flatMap((r) => r?.recommendNodeIds || []))
  );
  const relatedNodeIds = Array.from(
    new Set(recommendations.flatMap((r) => r?.relatedNodeIds || []))
  );

  return (
    <div className="min-h-screen bg-gray-50">
      {/* ヘッダー */}
      <div className="bg-white border-b sticky top-0 z-10 shadow-sm h-20 flex items-center">
            ...
            <Card className="shadow-sm">
              ...
              <CardContent className="p-0">
                ...
                <div className="relative">
                  ...
                  {/* 行番号 */}
                  <div
                    className="absolute top-0 left-0 w-10 min-h-[220px] bg-gray-100 border-r text-right pr-2 pt-2 text-xs text-muted-foreground font-mono select-none pointer-events-none"
                    style={{ lineHeight: "1.5" }}
                  >
                    {code.split("\n").map((_, i) => (
                      <div key={i}>{i + 1}</div>
              ...
              <CardContent className="py-2">
                <Tabs value={srkTab} onValueChange={setSrkTab}>
                  <TabsList className="grid w-full grid-cols-4 mb-2 h-8">
                    ...
                    <TabsTrigger value="skill" className="text-xs py-1">
                      Skill
                    </TabsTrigger>
                    <TabsTrigger value="rule" className="text-xs py-1">
                      Rule
                    </TabsTrigger>
                    <TabsTrigger value="knowledge" className="text-xs py-1">
                      Knowledge
                    </TabsTrigger>
                  </TabsList>
                  ...
                  <TabsContent value="overview" className="space-y-2 mt-2">
                    <p className="text-xs text-muted-foreground">
                      現在のエラー：Skill {skillErrors.length}件 / Rule{" "}
                      {ruleErrors.length}件 / Knowledge {knowledgeErrors.length}
                      件
                    </p>
                    ...
                    knowledgeErrors.length === 0 ? (
                      ...
                      <div className="space-y-1.5 max-h-[180px] overflow-y-auto">
                        {skillErrors.slice(0, 2).map((error, index) => (
                          ...
                          >
                            ...
                            {error.relatedNodeIds &&
                              error.relatedNodeIds.length > 0 &&
                              onStartLearning &&
                              (() => {
                                const relatedNode = learningNodesArray.find(
                                  (n) => n.id === error.relatedNodeIds![0]
                                );
                                return relatedNode ? (
                                  <Button
                                    variant="outline"
                                    ...
                                    onClick={() =>
                                      onStartLearning(error.relatedNodeIds![0])
                                    }
                                  >
                        ...
                        {ruleErrors.slice(0, 2).map((error, index) => (
                          <div
                            key={index}
                            className="p-2 bg-orange-50 rounded border"
                          >
                            <div className="flex items-start gap-2 mb-1">
                              <Badge
                                variant="outline"
                                className="text-xs border-orange-300 text-orange-700 py-0 h-4"
                              >
                                Rule
                              </Badge>
                              <span className="text-xs flex-1">
                        ...
                        {knowledgeErrors.slice(0, 2).map((error, index) => (
                          ...
                          >
                            <div className="flex items-start gap-2 mb-1">
                              <Badge
                                variant="outline"
                                className="text-xs border-blue-300 text-blue-700 py-0 h-4"
                              >
                                Knowledge
                              </Badge>
                              <span className="text-xs flex-1">
                  ...
                  <TabsContent
                    value="skill"
                    className="space-y-2 mt-2 max-h-[180px] overflow-y-auto"
                  >
                    <h5 className="text-xs">
                      Skillエラー（タイピングや記号の抜けなど）
                    </h5>
                    {skillErrors.length === 0 ? (
                      <div className="text-center py-4 text-xs text-muted-foreground">
                        Skillエラーは検出されませんでした！
                    ...
                    {knowledgeErrors.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {knowledgeErrors.map((error, index) => (
                          <div
                            key={index}
                            className="p-2 border rounded-lg bg-blue-50"
                          >
                            {error.line && (
                              <div className="flex items-center gap-2 mb-1">
                                <Badge
                ...
                currentNode.prerequisites.length > 0 ? (
                  <>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {currentNode.prerequisites.map((prereqId: string) => {
                        const prereqNode = learningNodesArray.find(
                          (n) => n.id === prereqId
                        );
                        ...
                        return (
                          <Badge
                            ...
                            onClick={() =>
                              setSelectedNodeForPreview(
                                selectedNodeForPreview === prereqId
                                  ? null
                                  : prereqId
                              )
                            }
                    ...
                    {selectedNodeForPreview &&
                      (() => {
                        const selectedNode = learningNodesArray.find(
                          (n) => n.id === selectedNodeForPreview
                        );
                        ...
                        return (
                          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                onStartLearning &&
                                onStartLearning(selectedNodeForPreview)
                              }
                            >
                              <BookOpen className="w-3 h-3 mr-1" />
                              この単元を復習する
                ...
                <CardContent className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {recommendations.slice(0, 3).map((rec, index) => (
                        <div
                          key={index}
                          className="flex items-start gap-2 p-1.5 bg-white rounded border"
                        >
                          <Badge
                            variant="outline"
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {recommendNodeIds.slice(0, 3).map((nodeId, index) => {
                          const node = learningNodesArray.find(
                            (n) => n.id === nodeId
                          );
                          if (!node) return null;
                          return (
                            <div
                              ...
                              onClick={() =>
                                onStartLearning && onStartLearning(nodeId)
                              }
                            >
                              <div className="flex items-center justify-between">
                  ...
                  {relatedNodeIds.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {relatedNodeIds
                          .filter((id) => !recommendNodeIds.includes(id))
                          .slice(0, 2)
                          .map((nodeId, index) => {
                            const node = learningNodesArray.find(
                              (n) => n.id === nodeId
